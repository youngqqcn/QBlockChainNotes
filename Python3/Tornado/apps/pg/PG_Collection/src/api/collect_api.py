...
from src.api.handlers.query_collection_data import QueryCollectionData
...
class MainHandler(BaseHandler):
    """
       @api {GET或POST} http://192.168.10.174/api/collection/ping  4.1.ping测试
       @apiVersion 0.0.1
       @apiName  ping
...
def make_app():
    application = tornado.web.Application([
        (r"/", MainHandler),
        (r"/ping", MainHandler),
        (r"/getcollectiondata", QueryCollectionData)
...
def main():
    ...
    tornado.options.define("port", default="59003", help="the port to listen", type=int)
