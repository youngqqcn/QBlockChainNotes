...
class Project(models.Model):
    ...
    pro_name = models.CharField(max_length=50, verbose_name='项目方', null=False, unique=True, help_text='项目方')
    tel_no = models.CharField(max_length=11,  verbose_name='手机号', null=False, help_text='手机号')
    email = models.EmailField(max_length=20, null=False,verbose_name='邮箱', help_text='邮箱')
    api_key = models.CharField(max_length=64, verbose_name='API_KEY', null=False, help_text='API_KEY')
    account_status = models.IntegerField(verbose_name='状态', choices=type_choices, null=False, help_text='状态')
    bip44_account_index = models.IntegerField(verbose_name='BIP44的账户索引', help_text='BIP44的账户索引',  null=False)
    create_time = models.DateTimeField(verbose_name='创建时间', null=False, help_text='创建时间')

    client_sign_key = models.CharField(max_length=100, verbose_name='客户端私钥', null=False, default='', help_text='客户端私钥')
    client_verify_key = models.CharField(max_length=100, verbose_name='客户端公钥', null=False, default='', help_text='客户端公钥')
    server_sign_key = models.CharField(max_length=100, verbose_name='服务端私钥', null=False, default='', help_text='服务端私钥')
    server_verify_key = models.CharField(max_length=100, verbose_name='服务端公钥', null=False, default='', help_text='服务端公钥')

    last_login = models.DateTimeField(verbose_name='上次登录时间', null=True, help_text='上次登录时间')
    password = models.CharField(max_length=80, verbose_name='密码', help_text='密码',
...
class AddAddressOrder(models.Model):
    ...
    order_id = models.CharField(max_length=30, verbose_name='订单号', primary_key=True, null=False, help_text='订单号')
    token_name = models.CharField(max_length=20, verbose_name='币种',null=False,help_text="币种")
    apply_times = models.IntegerField(verbose_name='申请次数', null=False, help_text='申请次数')
    count = models.IntegerField( verbose_name='申请数量', null=False, help_text='申请数量')
    start_addr_index = models.IntegerField(verbose_name='本次生成,起始地址索引', null=False, help_text='本次生成,起始地址索引')
    end_addr_index = models.IntegerField(verbose_name='本次生成,结束地址索引', null=False, help_text='本次生成,结束地址索引')
    audit_status = models.CharField(max_length=20, verbose_name='审核状态', choices=audit_status_choices, help_text='审核状态')
    generate_status = models.CharField(max_length=20, verbose_name='地址生成状态', choices=generate_status_choices, help_text='地址生成状态')
    order_create_time = models.DateTimeField(verbose_name='订单生成时间', null=True, help_text='订单生成时间')
    audit_complete_time = models.DateTimeField(verbose_name='审核完成时间', auto_now=True, help_text='审核完成时间')
    order_complete_time = models.DateTimeField(verbose_name='订单完成时间', null=True, help_text='订单完成时间')
    order_status = models.CharField(max_length=20, verbose_name='订单状态',choices=order_status_choices, null=False, help_text='订单状态')
    remark = models.CharField(max_length=250, verbose_name='备注(remark)', null=True, help_text='备注(remark)')
    #注意:  地址生成后需要, 刷新到redis地址池中, 才能进行充币监控   by yqq
    active_status = models.CharField(max_length=10, verbose_name='启用状态', choices=active_status_choices, null=False, help_text='启用状态')
    pro_id = models.ForeignKey(Project, null=False, verbose_name='项目方ID', on_delete=models.CASCADE, db_column="pro_id", help_text='项目方ID')
...
class WithdrawConfig(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    pro_id = models.ForeignKey(Project,on_delete=models.CASCADE, db_column="pro_id",verbose_name="项目方ID", help_text='项目方ID')
    ...
    is_open = models.BooleanField(verbose_name='提币通道开启状态',null=False, default=True, help_text='提币通道开启状态')
...
class DjangoAdminLog(models.Model):
    action_time = models.DateTimeField(help_text='操作时间', )
    object_id = models.TextField(blank=True, null=True, help_text='被操作用户ID')
    object_repr = models.CharField(max_length=200, help_text='被操作用户名')
    action_flag = models.PositiveSmallIntegerField(help_text='操作状态')
    change_message = models.TextField(help_text='操作内容')
    content_type = models.ForeignKey('DjangoContentType', models.DO_NOTHING, blank=True, null=True, help_text='操作功能')
    user = models.ForeignKey(User, models.DO_NOTHING, help_text='操作者')
