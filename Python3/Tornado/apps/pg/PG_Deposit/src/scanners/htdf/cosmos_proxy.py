...
from src.scanners.htdf.my_bech32 import Bech32AddrToHex
...
class CosmosProxy(object):
    '''
    CosmosProxy rpc proxy
    '''

    def __init__(self, host='localhost', port=1317, tls=False, cointype=''):
        self.host = host
        self.port = port
        self.tls = tls
    ...
    def _post_json_call(self,  subPath , params):
        ...
        try:
            ...
            errMsg =  "node server return nothing, maybe your arguments is invalid. error:{}".format(e)
    ...
    def _get_url_call(self, strUrl):
        ...
        try:
            ...
            r.encoding = 'utf-8'  #解决memo是一个汉字时,乱码的bug
        except RequestsConnectionError:
            raise BadConnectionError("request  node failed, please contact system administrator to check node server.") #fixed bug, 2019-04-13 yqq
        if r.status_code != 200:
    ...
    def getBalance(self, strAddr):
        ...
        #之前有过交易的, 但是钱转完了,一毛钱不剩
        if not retList: return "0"
    ...
    def getTxValidInfo(self, strTxid):
        urlpath = 'txs/' if self.cointype.upper() == 'HTDF' else  'transaction/'
        jsonText = self._get_url_call(urlpath + str(strTxid))
        try:
            if self.cointype.upper() == 'HTDF':
    ...
    def isValidTx(self, strTxid):
        ...
        try:
            #2019-07-13 htdf升级之后log改为了json对象
            #2020-02-25  log 改为了 logs
            strLog = str(json.dumps(jobj["logs"]))  if self.cointype.upper() == 'HTDF' else str(json.dumps(jobj["log"]))
            ...
            raise BadJsonError(f"json error: {e}" + str(jsonText))
