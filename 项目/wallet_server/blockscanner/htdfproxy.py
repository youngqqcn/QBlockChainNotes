...
class USDPProxy(object):
    ...
    def _post_json_call(self,  subPath , params):
        ...
        try:
            ...
            raise BadConnectionError("request htdf node failed, please contact system administrator to check node server.") #fixed bug, 2019-04-13 yqq
        else:
            errMsg = r.text if len(r.text) != 0 else "node server return nothing, maybe your arguments is invalid."
            raise BadConnectionError("error. %s"% errMsg) 
            
        if r.status_code != 200:
            raise BadStatusCodeError("error: %d , %s" %(r.status_code, r.text))
        try:
            return r.text
        except ValueError:
    ...
    def newaccount(self, strPwd):
        '''
        创建地址
        '''
        return  self._post_json_call('accounts/newaccount', {"password":strPwd})
    ...
    def getBalance(self, strAddr):
        ...
        for item in retList:
            if item["denom"] ==  "usdp" or item["denom"] == "htdf":
