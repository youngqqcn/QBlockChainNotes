...
from constants import ETC_IP_ADDR,ETC_RPC_PORT,ETH_BLK_BUFFER_SIZE
import  sql
...
ip_addr, port = ETC_IP_ADDR,ETC_RPC_PORT
...
class ETH_GetBalance(BaseHandler):
    ...
    def get(self):
        ...
        try:
            ...
            data = ETH_GetBalance.get_all_balance(rpcconn, address)
            strbalance = data['ETC']
            self.write(json.dumps(BaseHandler.success_ret_with_data(strbalance), default=decimal_default))
        except Exception as e:
            self.write(json.dumps(BaseHandler.error_ret_with_data("error: %s" % e)))
            logging.error("ETH_GetBalance error:{0} in {1}".format(e, get_linenumber()))
...
class ETH_SendRawTransaction(BaseHandler):
    ...
    def get_order_from_db(self, order_id):
        import sql
        sqlRet = sql.run("select * from tb_etc_broadcast where order_id='{0}';".format(order_id))
    ...
    def insert_txid_into_db(self, order_id, txid):
        import sql
        strSql = """insert into tb_etc_broadcast(order_id, txid) values('{}','{}');""".format(order_id, txid)
...
class ETH_ListAccounts(BaseHandler):
    ...
    def addresses():
        from sql import run
        accounts = run("""select address from t_eth_accounts;""") #共用 ETH的地址
...
class ETH_CrawlTxData(BaseHandler):
    ...
    #@staticmethod
    def process(self,  nStart, symbol='ETC'):
...
class ETH_CollectionQueryEx(BaseHandler):
    ...
    def proccess(self, rpcconn , symbol):
        ...
        strSql = """SELECT  address FROM tb_etc_series_active_addrs WHERE `symbol`='{}' AND `balance` > 0.0001 ORDER BY `balance` DESC LIMIT 100;""".format(symbol)
        sqlRet = sql.run(strSql)
        addrs = []
        for item in sqlRet:
            if "address" in item:
