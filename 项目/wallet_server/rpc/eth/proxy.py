...
#from constants import  ETH_GETBALANCE_API_URL 
...
class EthereumProxy(object):
    ...
    def _call_proxy(self, method, params=None, _id=1):
        ...
        if method == 'eth_getBlockByNumber':
            ...
            if len(params) == 0: 
                raise BadJsonError("args error.")
            strParams = "api?module=proxy&action=eth_getTransactionCount&address={0}&tag=latest&apikey=CMBN61ZYD517UW359EVRFCNX1MN48W37N2".format(str(params[0])) 
        elif method == 'eth_sendRawTransaction':
            if len(params) == 0: 
                raise BadJsonError("args error.")
            strParams = "api?module=proxy&action=eth_sendRawTransaction&hex={0}&apikey=CMBN61ZYD517UW359EVRFCNX1MN48W37N2".format(str(params[0])) 
            pass
        elif method == 'eth_getBalance':
            if len(params) == 0:
                raise BadJsonError("args error.")
            strParams = 'v1/eth/main/addrs/{0}/balance'.format(params[0])
            pass

        url = ''
        if method == 'eth_getBalance':
            url = 'https://{}/{}'.format( ETH_GETBALANCE_API_URL, strParams)
        else:
    ...
    def _call(self, method, params=None, _id=1):
        ...
        #使用备用的代理方案
        #if ("etherscan.io" in self.host) or ('eth_getBalance' == method):
