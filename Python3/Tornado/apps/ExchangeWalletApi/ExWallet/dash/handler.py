...
from constants import DASH_RPC_URL as RPC_URL
STR_ADDRESS_TABLE = "t_dash_address"
...
class BTC_EstimateSmartFee(BaseHandler):
    ...
    def process(rpcconn, nConfTarget=2, strEstimateMode='ECONOMICAL'):
        # commands = [["estimatefee", nConfTarget]]
...
class BTC_CreateRawTransactionEx_Collection(BaseHandler):
    ...
    def makeParams( rpcconn, lstSrc, lstDest):
        ...
        if isinstance(lstSrc, list):
            ...
            #DASH不支持 option
            # bitcoin-cli -conf=/root/.bitcoin/bitcoin-test.conf listunspent 0 9999999 '[]' true
            commands = [["listunspent", 1, 99999999, [], True]]
