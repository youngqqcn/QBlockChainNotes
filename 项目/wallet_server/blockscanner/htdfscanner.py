...
from htdfproxy import USDPProxy
import traceback
import json
import sql
import time

from config import  HTDF_NODE_RPC_HOST 
from config import  HTDF_NODE_RPC_PORT

#from config import  HTDF_N_DEFAULT_START_HEIGHT 
from config import  HTDF_N_BLOCK_TO_WAIT_CONFIRM   #区块确认数至少12个
#算法
#0.等待15秒
...
class USDPBlockScanner(USDPProxy):
    ...
    def GetMaxBlockNumberFromDB(self):
        try:
            strSql = """select MAX(height) from t_htdf_charge;"""
            #print(strSql)
    ...
    def PushTxDataIntoDB(self, nHeight, txDatas):
        ...
        if not isinstance(nHeight, int):
            print("nHeight is not integer.")
            return None

        try:
            jsonTxStr = json.dumps(txDatas)
            #print(jsonTxStr)
            strSql = """REPLACE INTO t_htdf_charge(height, txdata) VALUES({0}, '{1}')""".format(nHeight, jsonTxStr)
            #print(strSql)
            sqlRet = sql.run(strSql)
            #print(sqlRet)
            return True
    ...
    def GetTxDataFromDB(self, nBegin, nEnd):
        ...
        try:
            ...
            strSql = """SELECT txdata FROM t_htdf_charge WHERE  height >= {0} and height <= {1};""".format(nBegin, nEnd)
            #print(strSql)
            sqlRet = sql.run(strSql)
            #print(sqlRet)
    ...
    def GetExchangeUserAddress(self):
        sqlRet = sql.run('select address from t_htdf_accounts;')
        addrs = []
        for item in sqlRet:
    ...
    def ScanBlock(self):
        ...
        nEnd = nChainLastest - HTDF_N_BLOCK_TO_WAIT_CONFIRM 

        #获取起始扫描区块高度
        from utils import GetScanStartBlock
        HTDF_N_DEFAULT_START_HEIGHT = GetScanStartBlock('htdf')      

        nStart = nMaxHeightDB
        if 0 == nMaxHeightDB or nMaxHeightDB < HTDF_N_DEFAULT_START_HEIGHT :
            nStart = HTDF_N_DEFAULT_START_HEIGHT 
        ...
        if nCount > 1000:
            ...
            import sql
            sql.run("""update t_scan_start set start_block={0} where coin_type='{1}'""".format(nEnd, 'htdf'))
...
def main():
    print("start htdf block scanner.........")
    while True:
        try:
            scanner =  USDPBlockScanner(HTDF_NODE_RPC_HOST , HTDF_NODE_RPC_PORT )
