...
{ 
	...
	return sha3(bytesConstRef((bytes*)&_input)); 
}
...
inline std::string RLP2HexStr(const bytes input)
{
	...
	//std::cout << "signed:" << strHexTmp << std::endl;
	return strHexTmp;
}

inline  std::string Bin2HexStr(const unsigned char *pBin, uint len)
{
	std::string  strHex;
	for (int i = 0; i < len; i++)
	{
