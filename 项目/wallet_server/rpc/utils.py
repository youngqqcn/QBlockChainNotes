...
def recommended(all,amount):
    ...
    from decimal import Decimal    
    aggregate = Decimal('0')
    for utxo in [item for item in all if int(item["confirmations"]) >= 6 and float(item["amount"]) > 0.00001  ]:
...
def accumulate(all):
    aggregate = 0
    for item in all: aggregate += float(item["amount"]) if int(item["confirmations"]) >= 6 and (float(item["amount"]) > 0.0001 ) else 0
    return aggregate
