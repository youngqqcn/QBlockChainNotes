...
class USDP_SendRawTransaction(BaseHandler):
    def post(self):
        ...
        try:
            ...
            import sql 
            sqlRet = sql.run("select * from t_usdp_broadcast where orderId='{0}';".format(orderId))
            retData = {}
                    ...
                    for i in range(nTry):
                        ...
                        try:
                            ...
                            if not bSuccess:
                                ...
                                sql.run("delete from t_usdp_broadcast where orderId='{0}';".format(retData['orderId']))
                                bFlag =True #需要重新广播新发来的交易
                                return
                            retData["sure"] = True  #100%确定成功的
                            #print("udpate..")
                            sql.run("update t_usdp_broadcast set sure=1 where orderId='{0}';".format(retData['orderId']))
                            break
                        except Exception as e:
            ...
            if bFlag :
                ...
                for i in range(nTry):
                    ...
                    try:
                        ...
                        if  i ==  nTry - 1:  retData["sure"] = False #不确定的
                        continue
                sql.run("insert into t_usdp_broadcast(orderId,txid,sure) values('{0}','{1}', {2})".format(orderId, rsp["txhash"], 1 if retData['sure'] else 0 ))
...
class USDP_CollectionQuery(BaseHandler):
    ...
    def proccess(self, rpcconn):
        ...
        strSql = """SELECT address FROM t_usdp_active_addrs WHERE `balance` > 0.001 ORDER BY `balance` DESC LIMIT 100;""";
        sqlRet = sql.run(strSql)
        addrs = []
        for item in sqlRet:
            if "address" in item: 
        ...
        for strAddr in addrs: 
            ...
            if not retInfo : #如果账户一毛钱不剩, 返回None
                strSql = """DELETE FROM t_usdp_active_addrs WHERE address='{0}';""".format(strAddr)
