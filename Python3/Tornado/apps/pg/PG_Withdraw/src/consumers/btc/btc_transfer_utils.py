...
class BTCTransferUitl(BTCProxy):
    ...
    def search_utxo(self, addrs: list,  total_amount: Decimal ) -> (bool, Dict):
        ...
        for addr in addrs :
            ...
            for utxo in utxos:
                ...
                if value_in_satoshi < MIN_AVAILABLE_UTXO_VALUE_IN_SATOSHI: continue   #金额太小, 小于 10000satoshi, 不要
    ...
    def transfer(self, src_addrs_key_map: OrderedDict, dst_addrs_amount_map: dict ,
        ...
        is_enough, founded_utxos = self.search_utxo(addrs=source_addrs, total_amount=total_amount)
        if not is_enough:
