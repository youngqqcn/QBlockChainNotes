...
class EthBlockScanner(EthProxy):
        ...
        while True:
            try:
                ...
                for tx in blockInfo["transactions"]:
                    ...
                    if tx['to'] in self.exUserAddrs:  # 普通的ETH充币
                        # 普通的ETH转账
                        txTmp["txid"] = tx["hash"]
                        txTmp["from"] = tx["from"]
                        txTmp["to"] = tx["to"]
                        ...
                        for log in receipt['logs']:
                            ...
                            event_hash = topics[0]
                            if ERC20_TRANSFER_EVENT_HASH.replace('0x', '').lower() != event_hash.replace('0x',
                                                                                                         '').lower():
                                continue
                            ...
                            txTmp["value"] = "%.8f" % RoundDown(
                                Decimal(hex2Dec(strValue)) / Decimal(10 ** nDecimal))  # 单位转换
                            print("found tx: {}".format(txTmp))
                ...
                print("GetTransactionsInfoFromBlock(nHeight) error:{}".format(e))
                sleep(15)
                # return None
        pass
    ...
    def PushTxDataIntoDB(self, txDatas):
        ...
        try:
            for tx in txDatas:
                strSql = """INSERT INTO tb_eth_series_deposit(`txid`,`symbol`,`from`,`to`,`value`,`block_number`,`block_time`,`nonce`,`confirmations`) """
                strSql += """ VALUES('{}','{}','{}','{}','{}',{},{},{},{})  """\
                ...
                for addr in [tx['from'], tx['to']]:
                    if addr in self.exUserAddrs:
                        strSql = """INSERT INTO t_eth_patch_addrs(`address`) VALUES('{}')""".format( addr )
                        sqlRet = sql.run(strSql)
    ...
    def ScanProc(self, nStart : int, nEnd : int):
        ...
        print(f'thread:{threading.currentThread() } scanned {nStart} to {nEnd} , starting')
        for n in range(nStart, nEnd):
            # if (n - nStart) % 25 == 0:
            #     print(f' thread:{threading.currentThread()} scanning block {n} ')
            ...
            for iTry in range(10):
                if self.PushTxDataIntoDB( txRet):
                    break
        print(f'thread:{threading.currentThread() } scanned {nStart} to {nEnd}  finished')
        pass
    ...
    def ScanBlock(self):
        ...
        #获取起始扫描区块高度
        ETH_N_DEFAULT_START_HEIGHT = self.__GetScanStartBlock('eth')      
        ...
        #保存本次扫描的结束区块高度
        import sql
        strSql = """update t_scan_start set start_block={0} where coin_type='{1}';""".format(nEnd, 'eth')
        sql.run( strSql )
        sleep(15)
    ...
    def PatchForAddrsBalance(self):
        sqlRet = sql.run('SELECT DISTINCT address FROM t_eth_patch_addrs;')
        addrs = []
        for item in sqlRet:
            if 'address' not in item : continue
        ...
        def QueryBalanceProc(addrs):
            for strAddr in addrs:
                ...
                if Decimal(strBalance) > 0.001:
                    strSql = """REPLACE INTO tb_eth_series_active_addrs(`symbol`,`address`, `balance`) VALUES('ETH', '{0}', {1})""".format(strAddr, strBalance)
                    print("sql: {}".format(strSql))
                    sqlRet = sql.run(strSql)
                ...
                strSql = """DELETE FROM t_eth_patch_addrs WHERE address='{0}';""".format(strAddr)
                print("sql: {}".format(strSql))
                sqlRet = sql.run(strSql)
            pass
...
def main():
    ...
    print("start ethereum block scanner.........")
    scanner =  EthBlockScanner(ETH_NODE_RPC_HOST , ETH_NODE_RPC_PORT ) #不必每次都重新创建对象
    while True:
        try:
